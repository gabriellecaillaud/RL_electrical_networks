...
client = AlpyneClient(
    r"C:\Users\gabri\Documents\CS\2A\Pole_projet\RL et Anylogic\Export-Test\model.jar", verbose=True, port=51150)
    ...
    def actor_loss(self, probs, actions, adv, old_probs, closs):
        ...
        for pb, t, op, a in zip(probability, adv, old_probs, actions):
            ...
            s2 = tf.math.multiply(tf.clip_by_value(
                ratio, 1.0 - self.clip_pram, 1.0 + self.clip_pram), t)
            # print(f"s2{s2}")
            sur1.append(s1)
            sur2.append(s2)
        ...
        loss = tf.math.negative(tf.reduce_mean(
            tf.math.minimum(sr1, sr2)) - closs + 0.001 * entropy)
        # print(loss)
        return loss
...
for s in range(steps):
    ...
    if avg_reward > best_reward:
        ...
        agentoo7.actor.save('model_actor_{}_{}'.format(
            s, avg_reward), save_format="tf")
        agentoo7.critic.save('model_critic_{}_{}'.format(
            s, avg_reward), save_format="tf")
